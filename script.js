    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        ...
        const KNOWLEDGE_BASE = [{
            ...
            relatedLocations: ['siliguri', 'sukna', 'rongtong', 'tindharia', 'kurseong', 'ghum', 'batasia-loop',
                'darjeeling'
            ]
        }, {
            id: 'darjeeling',
        ...
        const state = {
            ...
            chatHistory: [], // { role, content, context? }
            isListening: false,
            isProcessing: false,
            demoMode: true, // always true for hackathon MVP
            currentView: 'home',
        ...
        function generateAnswer(question, heritageEntry) {
            ...
            // Check if question matches any common question
            for (const q of h.commonQuestions) {
                if (normalize(q) === qNorm || qNorm.includes(normalize(q).slice(0, 10))) {
                    // Return a detailed answer from the description
            ...
            // Generic fallback using the entry's content
            answer = `📍 ${name} — ${cat}\n\n${h.shortDescription}\n\n${h.detailedDescription.replace(/<p>|<\/p>/g, '').trim()}`;
            if (h.interestingFacts.length) {
        ...
        function processQuestion(question, source) {
            ...
            if (source === 'chat') {
                ...
                // Home → redirect to chat view with context
            ...
            setTimeout(() => {
                ...
                // Update current location context if relevant
        ...
        function renderHeritageList() {
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0 8px;">
              ...
              <span class="text-sm text-muted">${items.length} locations</span>
            </div>
        ...
        function updateAllContexts() {
            ...
            // Update route context if visible
            updateRouteContext();
            // Update demo badge
            const loc = getCurrentLocation();
            demoBadge.innerHTML = `<span class="icon">📍</span> Demo Journey · ${loc.name}`;
        }
        ...
        function updateOfflineStatus() {
            ...
            statusLabel.textContent = online ? 'Online' : 'Offline · using local data';
            if (!online) {
        ...
        function init() {
            ...
            // Set initial location to Batasia Loop
            state.currentLocation = 'batasia-loop';
            updateAllContexts();

            // Pre-populate chat with a welcome
            renderChat();
        ...
        if (window.location.hash) {
            const hash = window.location.hash.slice(1);
            if (views[hash]) setTimeout(() => navigateTo(hash), 100);
        }
